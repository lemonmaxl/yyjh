...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统框架 <span
        class="c-gray en">&gt;</span> 系统模块 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray "><span class="l"><!-- <?php if($funDEL): ?><a href="javascript:;" onclick="datadel()"
        ...
        &#xe6e2;</i> 删除</a><?php endif; ?> --><a href="javascript:;"
                                        onclick="admin_add('添加','<?php echo U('SysModule/add');?>','800','500')"
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <?php if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td style="text-align: left;">├<?php echo ($vo["delimiter"]); ?><a href="<?php echo U('SysModule/index',array('pid'=>$vo[id]));?>"
                                                                 style="color: blue"><?php echo ($vo["name"]); ?></a></td>
                ...
                <td class="td-manage">
                    <a title="编辑"
                       href="javascript:;"
                       onclick="admin_edit('编辑','<?php echo U('SysModule/edit');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                       class="ml-5"
                    ...
                    <a title="功能"
                       ...
                       class="ml-5"
                       style="text-decoration:none">功能</a>
...
<script type="text/javascript">
    function changeStatus(id) {
        ...
        $.ajax({
            ...
            url: "<?php echo U('SysModule/changeStatus');?>", // 需要提交的 url
            data: {
                'id': id,
                'status': dataStatus
            },
    ...
    function admin_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('SysModule/del');?>", // 需要提交的 url
                data: {
                    'id': id
                },
...
//            layer.msg('已删除!', {icon: 1, time: 1000});
    ...
    function admin_stop(obj, id) {
        layer.confirm('确认要停用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('SysModule/stop');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
    ...
    function admin_start(obj, id) {
        layer.confirm('确认要启用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('SysModule/start');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
