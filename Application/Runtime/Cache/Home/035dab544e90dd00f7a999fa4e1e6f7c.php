...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统管理 <span
        class="c-gray en">&gt;</span> 卫生室列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray "><span class="l"><!-- <?php if($funDEL): ?><a href="javascript:;" onclick="datadel()"
        ...
        &#xe6e2;</i> 删除</a><?php endif; ?> --> <a href="javascript:;" onclick="admin_add('添加','<?php echo U('Clinic/add');?>','800','500')"
    ...
    <table class="table table-border table-bordered table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="8">卫生室列表</th>
        </tr>
        <tr class="text-c">
            <th width="25"><input type="checkbox" name="" value=""></th>
            <th width="40">服务团队名称</th>
            <th width="40">UID</th>
            <th width="40">负责人</th>
            <th width="40">联系电话</th>
            <th width="40">操作员</th>
        ...
        <?php if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td>
                    <?php if(empty($vo["uerid"])): ?><span class="label label-success radius">否</span>
                    <?php else: ?> 
                        <span class="label label-danger radius">是</span><?php endif; ?> 
                </td>
                ...
                <td class="td-manage">
                    ...
                    <a title="编辑"
                       href="javascript:;"
                       onclick="admin_edit('编辑','<?php echo U('Clinic/edit');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                       class="ml-5"
                    ...
                    <a title="管理员"
                       href="javascript:;"
                       onclick="admin_edit('功能','<?php echo U('Clinic/admin');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                       class="ml-5" style="text-decoration:none">管理员</a>
                      <a title="生成二维码"
                       href="javascript:;"
                       onclick="admin_edit('功能','<?php echo U('Clinic/setqr');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                       class="ml-5" style="text-decoration:none">二维码</a>
                </td>
    ...
    function admin_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                ...
                url: "<?php echo U('Clinic/del');?>", // 需要提交的 url
                data: {
                    'id': id
                },
...
//            layer.msg('已删除!', {icon: 1, time: 1000});
    ...
    function admin_stop(obj, id) {
        layer.confirm('确认要停用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Clinic/stop');?>", // 需要提交的 url
                data: {
                    'id': id,
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
    ...
    function admin_start(obj, id) {
        layer.confirm('确认要启用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Clinic/start');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
