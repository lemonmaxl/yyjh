...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l"><a href="javascript:;" onclick="admin_add('添加','<?php echo U('Qylxset/object_add');?>',<?php echo ($id); ?>,'1100','600')"
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <?php if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                <input type="hidden" value="<?php echo ($vo["id"]); ?>" name="xmid[]" checked>
                ...
                <td><div  class="buy-num-bar buy-num clearfix">
                        ...
                        <input  name="num_<?php echo ($vo["id"]); ?>" id="num_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["buynum"]); ?>" size="4" maxlength='5' style="text-align:center;">
...
//            success: "valid",
            submitHandler: function (form) {
                $('#form-admin-add').ajaxSubmit({
                    ...
                    url: "<?php echo U('Qylxset/object_do');?>", // 需要提交的 url
                    data: {
 ...
 function admin_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Qylxset/object_del');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
