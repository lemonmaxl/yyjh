...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统管理 <span
        class="c-gray en">&gt;</span> 兴趣类型 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray "><span class="l"><!-- <?php if($funDEL): ?><a href="javascript:;" onclick="datadel()"
        ...
        &#xe6e2;</i> 删除</a><?php endif; ?> --> <a href="javascript:;" onclick="admin_add('添加','<?php echo U('Interest/type_add');?>','800','500')"
    ...
    <table class="table table-border table-bordered table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="4">兴趣类型</th>
        </tr>
        <tr class="text-c">
            <th width="25"><input type="checkbox" name="" value=""></th>
            <th width="100">兴趣类型</th>
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td class="td-manage">
                    ...
                    <a title="编辑"
                       href="javascript:;"
                       onclick="admin_edit('编辑','<?php echo U('Interest/type_edit');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                       class="ml-5"
    ...
    function admin_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                ...
                url: "<?php echo U('Interest/type_del');?>", // 需要提交的 url
                data: {
                    'id': id
                },
...
//            layer.msg('已删除!', {icon: 1, time: 1000});
    ...
    function admin_stop(obj, id) {
        layer.confirm('确认要停用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                    url: "<?php echo U('Interest/stop');?>", // 需要提交的 url
                data: {
                    'id': id,
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
    ...
    function admin_start(obj, id) {
        layer.confirm('确认要启用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Interest/start');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
