...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray "><span class="l"><!-- <?php if($funDEL): ?><a href="javascript:;" onclick="datadel()"
        ...
        &#xe6e2;</i> 删除</a><?php endif; ?> --> <a href="javascript:;" onclick="admin_add('添加','<?php echo U('Qylxset/add');?>','1000','700')"
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <?php if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td class="td-status">
                    <?php if($vo["sfsf"] == 0): ?><span class="label label-success radius">收费</span>
                        <?php else: ?>
                        <span class="label radius">免费</span><?php endif; ?>
                </td>
                <td class="td-status">
                ...
                <td class="td-manage">
                    ...
                    <a title="编辑"
                       href="javascript:;"
                       onclick="admin_edit('编辑','<?php echo U('Qylxset/edit');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                       class="ml-5"
                    ...
                    <a title="删除" href="javascript:;"
                       ...
                       style="text-decoration:none">删除</a>
                        ...
                        <?php if($vo["sfsf"] == 0): ?><a title="项目" href="javascript:;"
                           onclick="admin_object('项目','<?php echo U('Qylxset/object');?>',<?php echo ($vo["id"]); ?>,'800','500')" class="ml-5"
                           style="text-decoration:none">项目</a><?php endif; ?>
...
function changeStatus(id) {
    ...
    $.ajax({
        ...
        url: "<?php echo U('Qylxset/changeStatus');?>", // 需要提交的 url
        data: {
            'id': id,
            'status': dataStatus
        },
...
function call_back_xyxmnum(xyxmstr) {
        $.ajax({
            type: "POST",
            url: "<?php echo U('Qylxset/pay_object');?>",//+tab,
            data: {
                'xyxmstr': xyxmstr,
            },// 你的formid
            success: function (data) {
    ...
    function admin_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Qylxset/del');?>", // 需要提交的 url
                data: {
                    'id': id
                },
...
//            layer.msg('已删除!', {icon: 1, time: 1000});
    ...
    function admin_stop(obj, id) {
        layer.confirm('确认要停用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Qylxset/stop');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
    ...
    function admin_start(obj, id) {
        layer.confirm('确认要启用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Qylxset/start');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
