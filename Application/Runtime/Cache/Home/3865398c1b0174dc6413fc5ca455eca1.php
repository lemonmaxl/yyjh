...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray "><span class="l"><!-- <?php if($funDEL): ?><a href="javascript:;" onclick="datadel()"
        ...
        &#xe6e2;</i> 删除</a><?php endif; ?> --> <a href="javascript:;" onclick="admin_add('添加','<?php echo U('Hospital/add');?>','800','500')"
    ...
    <table class="table table-border table-bordered table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="9">医院列表</th>
        </tr>
        <tr class="text-c">
            <th width="25"><input type="checkbox" name="" value=""></th>
            <th width="40">名称</th>
            <th width="160">服务器地址</th>
            <th width="90">应用ID</th>
            <th width="140">应用SECRET</th>
            <th width="30">Token</th>
            <th width="30">管理员存在</th>
            <th width="50">操作</th>
        </tr>
        </thead>
        <tbody>
        <?php if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td class="td-manage">
                    ...
                    <!--<?php endif; ?>-->
                        ...
                        <a title="编辑"
                           href="javascript:;"
                           onclick="admin_edit('编辑','<?php echo U('Hospital/edit');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                           class="ml-5"
                    ...
                    <?php if($funadmin): ?><a title="管理员" href="javascript:;"
                           onclick="admin_info('编辑','<?php echo U('Hospital/admin');?>',<?php echo ($vo["id"]); ?>,'800','500')" class="ml-5"
                           style="text-decoration:none">管理员</a><?php endif; ?>

                    <?php if($funwechat): ?><a title="微信菜单"
                           href="javascript:;"
                           onclick="admin_edit('微信菜单','<?php echo U('Hospital/wmenu');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                           class="ml-5"
                           style="text-decoration:none">微信菜单</a><?php endif; ?>
                    <?php if($funyyh): ?><a title="医养护菜单"
                           href="javascript:;"
                           onclick="admin_edit('医养护菜单','<?php echo U('Hospital/yyhmenu');?>',<?php echo ($vo["id"]); ?>,'800','500')"
                           class="ml-5"
                           style="text-decoration:none">医养护菜单</a><?php endif; ?>
                </td>
    ...
    function admin_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                ...
                url: "<?php echo U('Hospital/del');?>", // 需要提交的 url
                data: {
                    'id': id
                },
...
//            layer.msg('已删除!', {icon: 1, time: 1000});
    ...
    function admin_stop(obj, id) {
        layer.confirm('确认要停用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Hospital/stop');?>", // 需要提交的 url
                data: {
                    'id': id,
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
    ...
    function admin_start(obj, id) {
        layer.confirm('确认要启用吗？', function (index) {
            ...
            $.ajax({
                type: 'post', // 提交方式 get/post
                url: "<?php echo U('Hospital/start');?>", // 需要提交的 url
                data: {
                    'id': id
                },
                success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
